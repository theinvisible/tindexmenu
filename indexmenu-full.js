...
dTree.prototype.add = function(dokuid, id, pid, name, hns, isdir, ajax) {
	this.aNodes[this.aNodes.length] = new Node(dokuid, id, pid, name, hns,
			isdir, ajax);
};
...
dTree.prototype.toString = function() {
	...
	if (this.config.scroll) {
		str += '<div id="cdtree_'
				+ this.obj
				+ '" class="dtree" style="position:relative;overflow:hidden;width:100%;">';
	}
	str += '<div id="dtree_' + this.obj + '" class="dtree ' + this.config.theme
			+ '" style="overflow:';
	if (this.config.scroll) {
	...
	if (this.config.toc) {
		str += '<div id="t' + this.obj + '" class="indexmenu_tocbullet '
				+ this.config.theme
				+ '" style="display:none;" title="Table of contents"></div>';
	...
	if (this.config.scroll) {
		...
		str += '<div id="left_'
				+ this.obj
				+ '" class="indexmenu_larrow" style="display:none;" title="Click to scroll back" onmousedown="javascript:'
				+ this.obj + '.scroll(\'r\',1);" onmouseup="javascript:'
				+ this.obj + '.stopscroll();"></div>';
		str += '</div>';
	}
	this.completed = true;
...
dTree.prototype.node = function(node, nodeId) {
	var h = 1, jsfnc, str;
	jsfnc = 'onmouseover="' + this.obj + '.show_feat(\'' + nodeId
			+ '\');" onmousedown="return indexmenu_checkcontextm(\'' + nodeId
			+ '\',' + this.obj
			+ ',event);" oncontextmenu="return indexmenu_stopevt(event)"';
	...
	node.icon = (this.root.id == node.pid) ? this.icon.root
			: ((node.hns) ? this.icon.folderH : ((node._hc) ? this.icon.folder
					: this.icon.node));
	node.iconOpen = (node._hc) ? ((node.hns) ? this.icon.folderHOpen
	...
	if (!node._hc || node.hns) {
		str += '<a id="s'
				...
				+ nodeId
				+ '" class="'
				+ ((node._cp) ? 'navSel' : ((node._is) ? 'nodeSel'
						: (node._hc) ? 'nodeFdUrl' : 'nodeUrl'));
		...
		str += '<a id="s' + this.obj + nodeId + '" href="javascript: '
				+ this.obj + '.o(' + nodeId + '); " class="node"' + jsfnc + '>'
				+ node.name + '</a>';
	} else {
		str += node.name;
	}
...
dTree.prototype.indent = function(node, nodeId) {
	...
	if (this.root.id != node.pid) {
		for (n = 0; n < this.aIndent.length; n++) {
			str += '<img src="'
					+ ((this.aIndent[n] == 1) ? this.icon.line
							: this.icon.empty) + '" alt="" />';
		}
		if (node._ls) {
			this.aIndent.push(0);
...
dTree.prototype.openCurNS = function(max) {
	...
	if (max > 0 && match.length >= max) {
		...
		n = (this.aNodes[0].dokuid == '') ? 0 : this.aNodes[0].dokuid
				.split(this.config.sepchar).length;
...
dTree.prototype.show_feat = function(n) {
	...
	if (this.config.scroll) {
		...
		if (dtreel > w) {
			...
			div.style.left = parseInt(node.offsetLeft + node.offsetWidth + w
					- 12, 0)
					+ "px";
...
dTree.prototype.getAjax = function(n) {
	...
	req = 'req=index&idx=' + node.dokuid
			+ decodeURIComponent(this.config.jsajax);
	curns = this.pageid.substring(0, this.pageid
			.lastIndexOf(this.config.sepchar));
	...
	Ajax.onCompletion = function() {
		...
		for (i = 0; i < ajxnodes.length; i++) {
			...
			ajxnode[1] += plus;
			selft.add(ajxnode[0], ajxnode[1], ajxnode[2], ajxnode[3],
					ajxnode[4], ajxnode[5], ajxnode[6]);
		}
		if (selft.fajax) {
			selft.fajax = false;
			selft.openCurNS(0);
...
dTree.prototype.loadCss = function() {
	...
	oLink.href = this.config.plugbase + '/images/' + this.config.theme
			+ '/style.css';
...
dTree.prototype.contextmenu = function(n, e) {
	...
	node = this.aNodes[n];
	self = this;
	rmenu.innerHTML = '<div class="indexmenu_rmenuhead" title="' + node.name
			+ '">' + node.name + "</div>";
	for (i = 0; i < cmenu.length; i++, i++, i++, i++) {
		if (((!node._hc || node.hns) && !cmenu[i + 2])
				|| (node._hc && !node.hns && !cmenu[i + 3])) {
			continue;
		}
		html = cmenu[i];
		if (cmenu[i + 1]) {
			...
			html = '<a title="' + cmenu[i] + '" href="' + eval(cmenu[i + 1])
					+ '">' + html + '</a>';
		} else {
			type = 'span';
